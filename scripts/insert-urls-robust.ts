...
// Updated direct Wikimedia URLs with corrected Python URL
...
console.log('Inserting imgURL fields for all 100 languages...\\n');

const languagesPath = path.join(__dirname, '../src/lib/data/languages.ts');
let content = fs.readFileSync(languagesPath, 'utf-8');

// Process each language ID
Object.entries(logoUrls).forEach(([langId, url]) => {
	// Look for the language object and color field
	const langPattern = new RegExp(
		`(\\s*{[\\s\\S]*?id:\\s*'${langId}'[\\s\\S]*?color:\\s*'[^']*',)(\\s*)([\\s\\S]*?})`,
		'g'
	);

	content = content.replace(langPattern, (match, beforeUrl, whitespace, afterUrl) => {
		// Check if imgURL already exists
		if (match.includes('imgURL:')) {
			// If it exists and is wrong, replace it
			if (langId === 'python' && match.includes('Lean_logo.svg')) {
				return match.replace(/imgURL:\\s*'[^']*'/, `imgURL: '${url}'`);
			}
			return match; // Keep existing correct URL
		}

		// Add imgURL after color with proper indentation
		return beforeUrl + `\\n\\t\\timgURL: '${url}',` + whitespace + afterUrl;
	});
...
console.log('\\n✅ All logo URLs inserted successfully!');
