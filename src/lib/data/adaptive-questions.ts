...
export function getAdaptiveQuestions(mbtiType: string): LanguageQuestion[] {
		...
		[
			{
				...
				languages: candidateLanguages
					.filter(l => 
						...
						['javascript', 'typescript', 'python', 'ruby', 'php', 'go', 'java', 'csharp', 'elixir', 'dart'].includes(l.id)
					)
					.map(l => l.id)
			},
			{
	...
	addQuestionIfValid(
		'style',
		'What programming style appeals to you?',
		'style',
		[
			{
				id: 'style_functional',
				...
				languages: candidateLanguages
					.filter(l => 
						...
						['haskell', 'elm', 'clojure', 'fsharp', 'ocaml', 'erlang', 'elixir', 'scala', 'purescript', 'reasonml'].includes(l.id)
					)
					.map(l => l.id)
			},
			{
				...
				languages: candidateLanguages
					.filter(l => 
						...
						['java', 'csharp', 'cpp', 'ruby', 'python', 'swift', 'kotlin', 'dart', 'smalltalk'].includes(l.id)
					)
					.map(l => l.id)
			},
			{
		...
		[
			{
				...
				languages: candidateLanguages
					.filter(l => 
						l.performanceNeeds === 'critical' || l.memoryManagement === 'manual' ||
			...
			},
			{
				...
				languages: candidateLanguages
					.filter(l => 
						l.performanceNeeds === 'flexible' || l.learningCurve === 'gentle' ||
	...
	addQuestionIfValid(
		'learning',
		'How do you feel about learning complexity?',
		'ecosystem',
		[
			{
				id: 'learn_easy',
	...
	addQuestionIfValid(
		'type_system',
		'How do you prefer to handle types in your code?',
		'style',
		[
			{
				id: 'type_static',
			...
			},
			{
				...
				languages: candidateLanguages
					.filter(l => 
						l.typing === 'gradual' ||
						['typescript', 'python', 'dart', 'kotlin'].includes(l.id)
					)
					.map(l => l.id)
			}
		]
	...
	addQuestionIfValid(
		'scale',
		'What scale of applications do you typically build?',
		'domain',
		[
			{
				id: 'scale_small',
				...
				languages: candidateLanguages
					.filter(l => 
						['python', 'bash', 'perl', 'ruby', 'lua', 'javascript'].includes(l.id)
					)
					.map(l => l.id)
			},
			{
			...
			},
			{
				...
				languages: candidateLanguages
					.filter(l => 
						['java', 'csharp', 'cpp', 'scala', 'go', 'rust', 'kotlin'].includes(l.id)
					)
					.map(l => l.id)
			}
		]
	...
	addQuestionIfValid(
		'platform',
		'Which platform is most important to you?',
		'domain',
		[
			{
				id: 'platform_browser',
				...
				languages: candidateLanguages
					.filter(l => 
						['javascript', 'typescript', 'elm', 'purescript', 'dart', 'reasonml', 'clojurescript'].includes(l.id)
					)
					.map(l => l.id)
			},
			{
			...
			},
			{
				...
				languages: candidateLanguages
					.filter(l => 
						['c', 'cpp', 'rust', 'ada', 'forth', 'assembly'].includes(l.id)
					)
					.map(l => l.id)
			}
		]
			...
			[
				{
					...
					languages: candidateLanguages
						.filter(l => ['python', 'javascript', 'java', 'cpp', 'csharp', 'typescript', 'go', 'swift', 'kotlin', 'rust'].includes(l.id))
						.map(l => l.id)
				},
				{
					id: 'pref_innovative',
				...
				},
				{
					...
					languages: candidateLanguages
						.filter(l => ['r', 'matlab', 'julia', 'sql', 'prolog', 'coq', 'agda', 'idris'].includes(l.id))
						.map(l => l.id)
				},
				{
					id: 'pref_classic',
					...
					languages: candidateLanguages
						.filter(l => ['c', 'lisp', 'fortran', 'cobol', 'pascal', 'ada', 'scheme', 'smalltalk'].includes(l.id))
						.map(l => l.id)
				}
			]
		);
